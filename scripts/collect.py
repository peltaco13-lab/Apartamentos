...
def detect_zone(text:str,zones):
    lower=text.casefold()
    for zone in zones:
        for alias in zone.get("aliases",[]):
            if alias.casefold() in lower:return zone["name"]
    return"Sur de Cali"
...
def listing_from_text(source_name,url,text,title,zones,max_rent,explicit_price=None):
    ...
    zone=detect_zone(combined,zones)
    if zone=="Sur de Cali":return None
    ...
    return{
        ...
        "zone":zone,"price":price,"bedrooms":first_number(combined,BED_PATTERNS),
...
def extract_json_ld(soup,source,page_url,zones,max_rent):
    found=[]
    for script in soup.find_all("script",attrs={"type":"application/ld+json"}):
        ...
        for obj in walk_json(payload):
            ...
            item=listing_from_text(source["name"],url,f"{title} {description}",title,zones,max_rent,ld_price(obj))
            if item:found.append(item)
    return found

def extract_anchors(soup,source,page_url,zones,max_rent):
    found=[]
    for anchor in soup.find_all("a",href=True):
        ...
        item=listing_from_text(source["name"],url,text,title or text[:120],zones,max_rent)
        if item:found.append(item)
    return found
...
def fetch_source(session,source,zones,max_rent):
    items,errors=[],[]
    for page_url in source.get("urls",[]):
        try:
            ...
            items+=extract_json_ld(soup,source,response.url,zones,max_rent)
            items+=extract_anchors(soup,source,response.url,zones,max_rent)
...
def main():
    ...
    config=load_json(CONFIG_PATH,{});previous=load_json(OUTPUT_PATH,{"listings":[]});site=config.get("site",{})
    zones,sources=site.get("zones",[]),config.get("sources",[]);max_rent=int(site.get("max_rent_cop",2_000_000));ttl=int(site.get("stale_ttl_days",14))
    ...
    for source in sources:
        ...
        status={"name":source["name"],"automated":automated,"ok":None,"found":0,"errors":[],"manual_urls":source.get("manual_urls",source.get("urls",[]))}
        if not automated or args.no_network:statuses.append(status);continue
        items,errors=fetch_source(session,source,zones,max_rent)
